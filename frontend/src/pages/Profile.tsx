...
const Profile: React.FC = () => {
  const [username, setUsername] = useState(() => {
    try {
      return localStorage.getItem('protube_username') || 'Mi canal';
    } catch {
      return 'Mi canal';
    }
  });
  ...
  useEffect(() => {
    const onStorage = () => {
      try {
        setVideos(JSON.parse(localStorage.getItem(STORAGE_KEY) || '[]'));
      } catch {
        setVideos([]);
      }
    };
  ...
  const [profile, setProfile] = useState<ChannelProfile>({
    title: localStorage.getItem('protube_username') || 'Mi canal',
  });
  ...
  useEffect(() => {
    const init = async () => {
      const identifier =
        localStorage.getItem('protube_username') ||
        localStorage.getItem('protube_user_id') ||
        localStorage.getItem('protube_user');
  ...
  const handleAvatarFile = (f: File | null) => {
    ...
    r.onload = () => setEditAvatarDataUrl(String(r.result || ''));
    r.readAsDataURL(f);
  };
  ...
  const [userComments, setUserComments] = useState<
    Array<{ id: string; videoId: string; titulo: string; descripcion: string }>
  >([]);
  const [receivedComments, setReceivedComments] = useState<
    Array<{ id: string; videoId: string; titulo: string; descripcion: string; userId: string }>
  >([]);
            ...
            <div className="postbox-header">
              ...
              <div style={{ display: 'flex', flexDirection: 'column', flex: 1 }}>
                <textarea
                  ...
                  onClick={() => {
                    if (!isLoggedIn) window.dispatchEvent(new CustomEvent('protube:open-login'));
                  }}
                  placeholder="Crea una publicació"
                  rows={2}
                />
                <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginTop: 8 }}>
                  <label className="postbox-tools" style={{ display: 'inline-flex', gap: 8, alignItems: 'center' }}>
                    <input
                      ref={(el) => {
                        postMediaInputRef.current = el;
                      }}
                      type="file"
                      accept="image/*,video/*"
                      style={{ display: 'none' }}
                      onChange={(e) => onChoosePostMedia(e.target.files ? e.target.files[0] : null)}
                    />
                    <button
                      type="button"
                      className="ghost"
                    ...
                    {postMedia && (
                      <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                        ...
                        <button type="button" className="ghost" onClick={() => setPostMedia(null)}>
                          Eliminar
                        </button>
                      </div>
                    )}
                ...
                {posts.map((p) => (
                  <div key={p.id} style={{ padding: 12, borderRadius: 8, background: '#fff' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      ...
                      <div>
                        <button className="action-btn ghost" onClick={() => deletePost(p.id)}>
                          Eliminar
                        </button>
                      </div>
                    </div>
                    ...
                    {p.video && (
                      <div style={{ marginTop: 8 }}>
                        <video src={p.video} controls style={{ maxWidth: '100%', borderRadius: 8 }} />
                      </div>
                    )}
                  </div>
                ))}
                ...
                {commentSubTab === 'made' && (
                  <div>
                    {userComments.length === 0 ? (
                      <div style={{ padding: 16, border: '1px solid #eef2f7', borderRadius: 8 }}>
                        <p style={{ margin: 0, color: '#6b7280' }}>No has fet cap comentari.</p>
                      </div>
                    ) : (
                      <div style={{ display: 'grid', gap: 12 }}>
                        {userComments.map((c) => {
                              ...
                              >
                                <div>
                                  <div style={{ fontWeight: 600, fontSize: 14 }}>
                                    ...
                                    {vInfo ? (
                                      <Link
                                        to={`/video/${encodeURIComponent(vInfo.fileName)}`}
                                ...
                                <button onClick={() => deleteUserComment(c.id)} className="action-btn ghost">
                                  Eliminar
                                </button>
                              </div>
                            </div>
                ...
                {commentSubTab === 'received' && (
                  <div>
                    {receivedComments.length === 0 ? (
                      <div style={{ padding: 16, border: '1px solid #eef2f7', borderRadius: 8 }}>
                        <p style={{ margin: 0, color: '#6b7280' }}>No has rebut cap comentari.</p>
                      </div>
                    ) : (
                      <div style={{ display: 'grid', gap: 12 }}>
        ...
        >
          <div style={{ width: 720, maxWidth: 'calc(100% - 40px)', background: '#fff', borderRadius: 12, padding: 18 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
              ...
              <button className="ghost" onClick={() => setCustomOpen(false)}>
                ✕
              </button>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 220px', gap: 12 }}>
              <div>
                ...
                <textarea
                  ...
                  style={{ width: '100%', minHeight: 100, padding: 8, borderRadius: 8, border: '1px solid #e6e6e6' }}
                />
              </div>
              <div style={{ textAlign: 'center' }}>
                ...
                >
                  {editAvatarDataUrl ? (
                    ...
                    <div style={{ color: '#9ca3af' }}>No avatar</div>
                  )}
                </div>
                <input
                  ref={(el) => {
                  ...
                  accept="image/*"
                  style={{ display: 'none' }}
                  onChange={(e) => handleAvatarFile(e.target.files ? e.target.files[0] : null)}
                />
                <div style={{ display: 'flex', gap: 8, justifyContent: 'center' }}>
                  <button className="action-btn" onClick={() => avatarInputRef.current?.click()}>
                    Tria avatar
                  </button>
                  <button className="action-btn ghost" onClick={() => setEditAvatarDataUrl(undefined)}>
                    Elimina
                  </button>
                </div>
              </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 12 }}>
              ...
              <button className="action-btn" onClick={saveProfile}>
                Desa
              </button>
            </div>
          </div>
